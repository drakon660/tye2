...
namespace Tye2.E2ETests
{
    public class TyePurgeTests
    {
        ...
        private string[] GetAllReplicasNames(Tye2.Hosting.Model.Application application)
        {
            var replicas = application.Services.SelectMany(s => s.Value.Replicas);
            return replicas.Select(r => r.Value.Name).ToArray();
        }

        private int[] GetAllAppPids(Tye2.Hosting.Model.Application application)
        {
            var replicas = application.Services.SelectMany(s => s.Value.Replicas);
        ...
        private string[] GetAllContainerIds(Tye2.Hosting.Model.Application application)
        {
            var replicas = application.Services.SelectMany(s => s.Value.Replicas);
